(ns deliveroo.main
  "Kotodama WASM entrypoint for the Deliveroo clean-room actor (L4) — Clojure port.
  ...
  facts (`deliveroo.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "deliveroo")
(def tier "L4")
...
(def entity-specs
  [{:entity "Order"      :plural "orders"      :id-prefix "delivero_ord"
   ...
   {:entity "Tracking"   :plural "trackings"   :id-prefix "delivero_tra"
...
(defn emit-facts
  "EAVT facts for one record: {\"deliveroo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "deliveroo-compat" :tier tier :entities entities} 200])
...
(defn start! [] :deliveroo-compat/ready)
